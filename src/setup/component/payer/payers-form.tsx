...
import {getPayerList} from './payerActions';

export const PAYERS_FORM = 'payersForm';

type PayersPropsType = SearchState<Payer> & SearchDispatchProps<Payer, PayerSM> & HistoryProps;

class PayersComponent extends ReduxSearchComponent<Payer, PayerSM, PayersPropsType, SearchState<Payer>> {
  constructor(props) {
  ...
  approve = (e: any, entityId: string) => {
    e.preventDefault();
    this.props.history.push('payer/approve/' + entityId);
  }
  ...
  render() {
    ...
    return (
      <div className='view-container'>
        <header>
          <h2>{resource.payers_list}</h2>
          {this.addable && <button type='button' id='btnNew' name='btnNew' className='btn-new' onClick={this.add} />}
        </header>
        <div>
          <form id={PAYERS_FORM} name={PAYERS_FORM} noValidate={true} ref={this.ref}>
              ...
              <table>
                <thead>
                  <tr>
                    ...
                    <th data-field='entityName'><button type='button' id='sortCompanyName' onClick={this.sort}>{resource.company_name}</button></th>
                    <th data-field='shortName'><button type='button' id='sortCompanyShortName' onClick={this.sort}>{resource.company_short_name}</button></th>
                ...
                <tbody>
                  {results.map((payer, i) => {
                    return (
                      <tr key={i}>
                        <td className='text-right'>{payer.sequenceNo}</td>
                        <td>{payer.entityName}</td>
                        <td>{payer.shortName}</td>
                        <td>{payer.ctrlStatusName}</td>
                        <td>{payer.actedBy}</td>
                        <td>{payer.actionDate}</td>
                        <td>{payer.actionStatus}</td>
                        <td>
                          {(this.editable || this.viewable) &&
                            <button type='button' id={'btnView' + i} className={this.editable ? 'btn-edit' : 'btn-view'} onClick={(e) => this.edit(e, payer.entityId)} />}
                          {this.approvable && payer.ctrlStatus === 'P' &&
                            <button type='button' id={'btnApprove' + i} className='btn-approve' onClick={(e) => this.approve(e, payer.entityId)} />}
                        </td>
                      </tr>
                    );
                  })}
...
export const payersSelector = new ListGlobalStateSelector(PAYERS_FORM);

function mapStateToProps(state) {
  return {
    results: payersSelector.selectListData(state)
  };
}

function mapDispatchToProps(dispatch): SearchDispatchProps<Payer, PayerSM> {
  return {
    search: (data) => dispatch(getPayerList(data))
  };
}
