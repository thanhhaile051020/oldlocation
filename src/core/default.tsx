...
export default class DefaultWrapper extends BaseComponent<HistoryProps, InternalState> {
  constructor(props) {
    ...
    this.httpRequest = new HttpRequest(axios, httpOptionsService);
  }
  ...
  renderForms = (features, isPinnedModules: boolean = false) => {
    return (
  ...
  renderForm = (key: any, module: any, isPinnedModules: boolean = false) => {
    const name = (!this.resource[module.resourceKey] || this.resource[module.resourceKey] === '') ? module.name : this.resource[module.resourceKey];
    // if (module.status !== 'A') {
    //   return (null);
    // }
