...
export class RoleAssignmentsForm extends SearchComponent<Role, RoleSM, HistoryProps, SearchState<Role>> {
  constructor(props) {
    super(props, applicationContext.roleAssignmentService, storage.resource(), storage.ui(), showToast, alertError, getLocale, storage.loading());
  ...
  edit = (e: any, id: string, cId: string) => {
    e.preventDefault();
    this.props.history.push(`access-role-assignment/edit/${id}/${cId}`);
  }

  approve = (e: any, role: Role) => {
    e.preventDefault();
    this.props.history.push(`access-role-assignment/approve/${role.roleId}/${role.cId}`);
  }

  render() {
    const resource = this.resource;
    const { ctrlStatusList, userTypeList, assignedCtrlStatus, userType, roleName } = this.state;
    return (
        <div className='view-container'>
          ...
          <div>
            <form id='roleAssignmentsForm' name='roleAssignmentsForm' noValidate={true} ref={this.ref}>
                    ...
                    {ctrlStatusList.map((item, index) => (
                            <label key={index}>
                              <input
                                  type='checkbox'
                                  id={item.value}
                                  name='assignedCtrlStatus'
                                  key={index}
                                  value={item.value}
                                  checked={assignedCtrlStatus.includes(item.value)}
                                  onChange={this.updateState}/>
                              {item.text}
                            </label>
                        )
                  ...
                  {this.state && this.state.results && this.state.results.map((role, i) => {
                    return (
                        <tr key={i}>
                          ...
                          <td>{role.roleId}</td>
                          <td>{role.roleName}</td>
                          <td>{role.assignedCtrlStatus}</td>
                          <td>{role.assignedBy}</td>
                          <td>{role.assignedDate}</td>
                          <td>{role.assignedStatus}</td>
                          <td>
                            ...
                            { this.approvable && role.assignedCtrlStatus === 'P' &&
